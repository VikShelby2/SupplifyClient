...
export default function ChannelsModal({openModal , setOpenModal  ,setInput , input ,  updateSales ,setBarcode , barcode , handleSave , variantChanges}){
    ...
    const toggleChannelStatus = () => {
        setInput(prev => {
            const newInput = { ...prev };
            newInput.publishing.channels[0].isActive = !prev.publishing.channels[0].isActive;
            return newInput;
        });
    };
    const toggleChannelStatusPos = () => {
        setInput(prev => {
            const newInput = { ...prev };
            newInput.publishing.channels[1].isActive = !prev.publishing.channels[1].isActive;
            return newInput;
        });
    };
    return(
        ...
        <ModalContent className="bg-[#fff]">
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1 text-[#1d1d1d]">Edit Sales Channel</ModalHeader>
              <ModalBody>
