    ...
    navMain: [
      ...
      },
      {
        ...
        items: [
          {
            title: 'Inventory',
            url: `/store-panel/products/inventory`,
            isActive: false,
          },
          {
            title: 'Collection',
            url: `/store-panel/collections`,
            isActive: false,
          },
  ...
  React.useEffect(() => {
    const updateActiveState = () => {
      const updatedNav = data.navMain.map((item) => {
        ...
        const mainActive = item.url === currentUrl || item.addUrl === currentUrl

        // Check if any subitem URL matches the current URL
        const subActive = item.items?.some(
        ...
        return {
          ...
          isSubActive: subActive, // Set isSubActive based on subitem match
              ...
              {data.navMain.map((item) =>
                item.isCollapsible ? (
                  <Collapsible
                    key={item.title}
                    asChild
                    defaultOpen={item.isActive || item.isSubActive}
                    ...
                    <SidebarMenuItem>
                      <CollapsibleTrigger asChild>
                        <Link to={item.url}>
                          <SidebarMenuButton
                            tooltip={item.title}
                            className={`${item.isActive ? 'bg-[#101010] text-white hover:bg-black hover:text-white' : ''}`}
                          >
                            {item.icon && <item.icon />}
                        ...
                        <SidebarMenuSub>
                          {item.items?.map((subItem) => (
                            <SidebarMenuSubItem key={subItem.title}>
                              <SidebarMenuSubButton
                                className={`${subItem.isActive ? 'bg-[#101010] text-white hover:bg-black hover:text-white' : ''}`}
                                asChild
                              >
                                <Link to={subItem.url}>
              ...
              {data.projects.map((item) => (
                <SidebarMenuItem key={item.name}>
                  ...
                  >
                    <Link to={item.url}>
                      <item.icon
                        className={` ${item.name === 'Online Store' ? 'text-violet-500' : ''}`}
                      />
                      <span>{item.name}</span>
                    </Link>
                  </SidebarMenuButton>
