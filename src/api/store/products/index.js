...
export const createProduct = (input , setProductId , setIsSaved) => async (dispatch) => {
...
export const listProducts = (store ) => async (dispatch) =>{
  ...
  console.log('start')
   const productReq = fetch('http://localhost:8080/api/product/list', {
      ...
      try {
        const [productResponse, tabsResponse] = await Promise.all([productReq, tabsReq]); 
...
export const editProduct =  (input ,setProductId , productId) => async (dispatch) =>{
  dispatch(setLoading(true));
  dispatch(setError(null));
  const formData = new FormData();

  for (const key in input) {
    formData.append(key, typeof input[key] === 'object' ? JSON.stringify(input[key]) : input[key]);
  }
 ...
 try {
    const response = await fetch(`http://localhost:8080/api/product/edit/${productId}`, {
    method: "PUT",
    body: formData
  });
...
export const handleAddCollection = async(storeId, query, tags, setOptions) =>{
  const name = { name: query };
    try {
      const response = await fetch(`http://localhost:8080/api/product/collection/add`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  export const handleAddTab =  ( name , storeId , table  , setOpen) => async ( dispatch) =>{
    try {
      const response = await fetch(`http://localhost:8080/api/product/tabs/add`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
export const handleEditTab = async( tabId , newName) =>{
  try {
    const response = await fetch(`http://localhost:8080/api/product/tabs/edit`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
export const handleListTabs =  (storeId  , type , resolve , reject) => async (dispatch) => {
  try {
    const response = await fetch(`http://localhost:8080/api/product/tabs/list`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
