...
export const HandleSignUp = async (input, navigate, dispatch, setStore) => {
  ...
  try {
    ...
    const userExists = accounts.some((account) => {
      try {
        const decryptedAccount = CryptoJS.AES.decrypt(
          account.user,
          SECRET_KEY
...
export const HandleLogIn = async (input, navigate, dispatch) => {
  dispatch(startLoading())
