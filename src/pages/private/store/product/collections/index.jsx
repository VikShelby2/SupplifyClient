...
import { handleAddTab, handleEditTab, handleListTabs } from '../../../../../api/store/products'
...
useEffect(() => {console.log(activeTab  , 'adasda ')}, [activeTab])
  ...
  const tabsConfig = useMemo(
    () => [
      ...
      ...tabs
      .filter((tab) => tab.table === 'collection-main-table')
      .map((tab) => ({
        name: tab.name,
        value: tab.name,
        id: tab._id,
  ...
  useEffect(() => {
    ...
    tabsConfig.forEach((tab) => {
      filtered[tab.value] = collections.filter(tab.filter)
    })
    setFilteredCollections(filtered)
  }, [collections, tabsConfig])
  return (
    <>
    <div className="w-full h-full flex items-center flex-col gap-10">
      <Header col={true} title={'Collections'} loading={loading} products={collections} />
      <main
      ...
      `}
              ...
              {tabsConfig.map((tab) => (
                <TabsContent
                  ...
                  key={tab.value}
                >
                  {filteredCollections[activeTab]?.length > 0 ? (
                    ...
                    <div className="items-center h-full relative justify-center overflow-x-auto w-full">
                      <Table
                        ...
                        filteredProducts={filteredCollections[activeTab]}
                      />
                    </div>
                  ) : (
