...
const [isCheckedRadioStateDetail, setIsCheckedRadioStateDetail] = useState('new'); // Initialize with default value
 ...
 const isSelectedProductId = useSelector((state)=> state.products.isSelectedProductId)
 const stateSelectedProduct = useSelector((state)=> state.products.selectedProduct)
 useEffect(()=>{
...
useEffect(() => {
  ...
  return (
    ...
    <div className={ `flex min-h-screen w-full flex-col  `} style={{cursor:'pointer'}}>
 
      <div className="flex flex-col sm:gap-4  mx-auto min-w-[1100px] max-w-[1100px] " style={{fontFamily:'sora, ui-sans-serif, system-ui, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji"'}}>
       
        <main className="grid flex-1 items-start gap-4   md:gap-8">
          <div className="mx-auto grid w-full flex-1 auto-rows-max gap-4">
          <Header store={store} productSaved={isSaved} titleFilled={titleValid} isProductIdSet={isProductIdSet} handelEditProduct={handelEditProduct} handleSubmit={handleSubmit}  />
          ...
          selectedGender={selectedGender}
            ...
            <div className="flex items-center justify-center gap-2 md:hidden">
              ...
              <Button style={{
                boxShadow:' 0rem -.0625rem 0rem .0625rem rgba(0, 0, 0, .8) inset, 0rem 0rem 0rem .0625rem rgba(48, 48, 48, 1) inset, 0rem .03125rem 0rem .09375rem rgba(255, 255, 255, .25) inset '
              }}
              size="sm">Save Product</Button>
            </div>
          </div>
        </main>
      </div>
    </div>
    <ChannelsModal setInput={setInput} input={input} handleSave={updateMyStore} openModal={openChannelsModal} setOpenModal={setOpenChannelsModal} updatePos={updatePos} />
    </DndProvider>
  )
}
