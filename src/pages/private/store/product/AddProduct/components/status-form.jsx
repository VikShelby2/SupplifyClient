...
export default function Status({handleStatusChange}){
    ...
    return(
      <>
       <Card x-chunk="dashboard-07-chunk-3" className="shadow-input rounded-[.75rem]" style={{background:'#fff' }}>
                  <CardHeader>
    ...
    >
        {(stat) => <SelectItem   key={stat.label} value={stat.label}>{stat.label}</SelectItem>}
    </Select>
                      </div>
                    </div>
                  </CardContent>
