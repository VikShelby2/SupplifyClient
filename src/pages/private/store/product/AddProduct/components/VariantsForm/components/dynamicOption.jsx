...
import WeightModal, {weights} from '../../../../../../../../components/admin/Modals/addProductModals/wight'
...
const DynamicOptions = ({ addedVariants, setAddedVariants }) => {
  ...
  const handleImageChange = (event, variantIndex, optionValueIndex) => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        updatedVariants[variantIndex].optionValues[optionValueIndex].photo = reader.result;
  ...
  const handleSave = () => {
    if (selectedOptions.length > 0) {
      ...
      selectedOptions.forEach(({ variantIndex, optionValueIndex }) => {
        const selectedOptionValue = updatedVariants[variantIndex].optionValues[optionValueIndex];
  ...
  const handleCheckboxChange = (variantIndex, optionValueIndex) => {
    ...
    const index = updatedSelectedOptions.findIndex(
      (item) => item.variantIndex === variantIndex && item.optionValueIndex === optionValueIndex
    );
  ...
  const handleHeaderCheckboxChange = () => {
    const allChecked = selectedOptions.length === addedVariants.reduce((acc, variant) => acc + variant.optionValues.length, 0);
    const allOptions = [];
  ...
  return (
    <>
      <div className="relative bg-[#fff] overflow-x-auto w-[99%] shadow-sm sm:rounded-b-lg ml-[0.1rem]">
        {addedVariants.length > 0 && (
          <table className="w-full text-sm text-left rtl:text-right text-black ">
            <thead className="text-xs text-black bg-[#fff] border-t-2 border-b-2 border-t border-b   ">
              <tr>
                <th scope="col" className="p-4" style={{ paddingTop: '.45rem', paddingBottom: '.45rem' }}>
                  <div className="flex items-center">
                    <Checkbox
                      ...
                      isSelected={selectedOptions.length === addedVariants.reduce((acc, variant) => acc + variant.optionValues.length, 0)}
                    />
                    <label htmlFor="checkbox-all-search" className="sr-only">checkbox</label>
                  </div>
                </th>
                <th scope="col" className="px-6 py-3" style={{ paddingTop: '.45rem', paddingBottom: '.45rem' }}>
                  Name
                </th>
                <th scope="col" className="px-6 py-3 text-right" style={{ paddingTop: '.45rem', paddingBottom: '.45rem' }}>
                 ...
                 <DropdownItem key="new" onClick={() => toggleModal('openModalPrice')}>
          Edit price
        </DropdownItem>
        <DropdownItem key="copy" onClick={() => toggleModal('openModalQuantities')} >
          Edit quantities
        </DropdownItem>
        <DropdownItem key="edit" onClick={() => toggleModal('openModalSKU')}>
          Edit SKU
        </DropdownItem>
        <DropdownItem key="new" onClick={() => toggleModal('openModalBarcode')}>
          Edit barcode
        </DropdownItem>
        <DropdownItem key="copy" onClick={() => toggleModal('openModalWeight')}>
          Edit weight
        </DropdownItem>
        <DropdownItem key="edit" onClick={() => toggleModal('openModalCountryRegion')}>
          Edit country/region
        </DropdownItem>
        <DropdownItem key="new" onClick={() => toggleModal('openModalHSCode')}>
          Edit HS code
        </DropdownItem>
        <DropdownItem key="copy" onClick={() => toggleModal('openModalAddImages')}>
          Add Images
        </DropdownItem>
        ...
        <DropdownItem key="edit" onClick={() => toggleModal('openModalEditLocation')}>
          Edit Location
        </DropdownItem>
                      ...
                      <td className="w-4 p-4">
                        <div className="flex items-center">
                          <Checkbox
                            ...
                            type="checkbox"
                            className="w-4 h-4 accent-black border-gray-300 rounded focus:ring-[#8c52fe] dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                            onChange={() => handleCheckboxChange(variantIndex, optionValueIndex)}
                            isSelected={isChecked(variantIndex, optionValueIndex)}
                          />
                          <label htmlFor={`checkbox-table-search-${variantIndex}-${optionValueIndex}`} className="sr-only">checkbox</label>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          <button
                            ...
                            style={{
                              boxShadow: 'rgba(255, 255, 255, 0.3) 0rem 0rem 0rem 0.0625rem inset',
                            }}
                            onClick={(e) => {
                              e.preventDefault();
                              document.getElementById(`dropzoneee-file-${variantIndex}-${optionValueIndex}`).click();
                            }}
                          >
                            {optionValue.photo ? (
                              <img src={optionValue.photo} alt="Selected" className="h-8 w-auto" />
                            ) : (
                              <Image />
                            )}
                          </button>
                          <input
                            ...
                            className="hidden"
                            multiple
                            onChange={(e) => handleImageChange(e, variantIndex, optionValueIndex)}
                          />
                          <div className="ps-3">
                            <div className="text-base font-semibold">{variant.optionName}-{optionValue.name}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-4 text-right">
                        <button className="pr-up-btn" onClick={() => handleEditClick(variantIndex, optionValueIndex)}>
                          Edit
                        </button>
                      </td>
                    </tr>
...
<path d="M12 5v14"></path>
     ...
     <PriceModal handleSave={handleSave} variantChanges={variantChanges} setOpenModal={setOpenModalPrice} openModal={openModalPrice} priceValue={price} compareValue={compareAt} setCompareAtValue={setCompareAt} setPriceValue={setPrice} handleFieldChange={handleFieldChange} />
     <SkuModal handleSave={handleSave} variantChanges={variantChanges} setOpenModal={setOpenModalSKU} openModal={openModalSKU} sku={sku} setSku={setSku} handleFieldChange={handleFieldChange} />
     <BarcodeModal  handleSave={handleSave} variantChanges={variantChanges}  setBarcode={setBarcode} barcode={barcode} setOpenModal={setOpenModalBarcode} openModal={openModalBarcode} handleFieldChange={handleFieldChange} />
     <WeightModal handleSave={handleSave} variantChanges={variantChanges}  setWeight={setWeight} weight={weight} setOpenModal={setOpenModalWeight} openModal={openModalWeight} handleFieldChange={handleFieldChange} />
     <QuantityModal handleSave={handleSave}  variantChanges={variantChanges}  setQuantity={setQuantity} quantity={quantity} setOpenModal={setOpenModalQuantities} openModal={openModalQuantities} handleFieldChange={handleFieldChange}/>
   </>
  );
};
