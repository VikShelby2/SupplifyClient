...
export default function ProductOrganizationForm({
       ...
       return(
        ...
        <Card 
        className="overflow-hidden bg-white shadow-input rounded-[.75rem]" x-chunk="dashboard-07-chunk-4"
      >
       
        <CardContent className="px-[12px] mt-[1.3rem]">
          <div className=" gap-2">
          <FormLabel className="text-black mb-2">Product Organization</FormLabel>
...
marginTop: '1rem'
            ...
            <button className='w-full'>   
              ...
              <Input  name="type"
               value={input.type}
               onChange={handleChange} type="text" id="small-input" 
            ...
            {selectedCollection.length > 0 && (
                 ...
                 selectedCollection.map((option) => ( <li className="pt-[.8rem]" >
            <div className="mt-2 p-[2px] pb-0 flex flex-wrap gap-2">
               <div key={option.id} className="bg-gray-100 px-2 py-1 rounded-lg flex items-center">
                 <span className="text-sm text-gray-800">{option.name}</span>
                 <button
                 onClick={() => {
                 setSelectedCollection(selectedCollection.filter((selected) => selected.id !== option.id));
                 }}
            ...
            <li className={`${selectedCollection.length > 0 ? 'pt-0' : 'pt-[.8rem]'}`}>
            <div class="relative h-10 w-full min-w-[200px]">
              <TagsCombobox isDropdownOpen={isDropdownOpen} setIsDropdownOpen={setIsDropdownOpen} selectedOptions={selectedTag} setSelectedOptions={setSelectedTag}/>
             </div>
            </li>
...
{selectedTag.length > 0 && (
  ...
  selectedTag.map((option) => ( <li className="" >
                         <div className="mt-2 p-[2px] pb-0 flex flex-wrap gap-2">
             <div key={option.id} className="bg-gray-100 px-2 py-1 rounded-lg flex items-center">
             <span className="text-sm text-gray-800">{option.name}</span>
             <button
             onClick={() => {
             setSelectedTag(selectedTag.filter((selected) => selected.id !== option.id));
             }}
             className="ml-1 text-gray-500 hover:text-[#8c52f1] focus:outline-none"
             >
             &times;
