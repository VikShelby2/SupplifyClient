...
useEffect(() => {
    ...
    filteredProducts();
  }, [selectedTags, selectedCat, selectedStatus, selectedVendor, priceRange, originalProducts, dispatch]);
  ...
  return (
      <div
          className='max-w-[450px] max-h-[350px]  w-full  grid flex-col items-center cursor-pointer  bg-white rounded-xl '>
          <div className={' w-full min-h-full  p-1 px-4 pt-5 pb-3 flex flex-col items-center'}>
              ...
              <div className='flex items-center justify-between w-full'>
                  ...
                  <Dropdown classNames={{
                      ...
                      <DropdownTrigger>
                          <div
                              className='p-1 cursor-pointer  border-dotted rounded-md flex items-center justify-between gap-2 px-2 border-2 text-[.75rem]'>
                              Add filter <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                      ...
                      <DropdownMenu
                          bottomContent={
                              <div
                                  className="px-2 cursor-pointer hover:text-violet-400 py-2 border-t border-default-300">
                                  Clear
                              </div>
                          }
                          classNames={{
                          ...
                          aria-label="Static Actions" variant="faded">
                          <DropdownSection className="p-1">
                              <DropdownItem className='text-[.75rem] ' key="new">Sales Channel</DropdownItem>
              ...
              <div className='flex items-cetner w-full flex-row gap-3'>
                  ...
                  <div className='flex w-full items-center'>
                      <Select
                          classNames={{
                              base: 'max-w-[204px]',
                              trigger: "border-[0.5px] data-[hover=true]:border-2 data-[focus=true]:border-2",
                          }}
                  ...
                  <div className='flex w-full items-center'>
                      ...
                      >
                          {productCategories.length > 0 && productCategories.map((item) => (
                              <SelectItem key={item.id}>{item.name}</SelectItem>
                          ))}
                      </Select>
                  </div>
          ...
          <div className={'flex w-full p-1 px-6 py-3 pt-2  items-center'}>
              <h1 className={`${products.length === originalProducts.length ? 'text-gray-300' : 'text-violet-600'}`}>clear</h1>
          </div>
      </div>
  )
}
