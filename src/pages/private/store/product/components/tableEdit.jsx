...
const TableEdit = ({ selected }) => {
  return (
    <div className="fixed left-1/2 bottom-0 transform -translate-x-1/2 p-4  text-center mb-10">
      <DraggableCom selected={selected} />
    </div>
  )
}

export const DraggableCom = ({ selected }) => {
  return (
    <Draggable>
      <div
        className="w-[400px] h-[40px] bg-white rounded-[.5rem] shadow-input p-2 flex items-center"
        style={{
          cursor: 'grab', // Change cursor style to indicate draggable behavior
        }}
      >
        <div className="w-full flex items-center justify-between text-black">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[.85rem]" style={{ fontWeight: '700' }}>
              {selected} selected
            </span>
          </div>
          <div>
