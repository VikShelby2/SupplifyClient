...
export const CustomCheckbox = ({item, value , noAvatar}) => {
    return (
        <Checkbox
            ...
            value={value}
        >
       {!noAvatar && (     <div className="w-full flex justify-between gap-2">
                <User
                    avatarProps={{size: "md" , icon : item.icon  , classNames: {icon: 'bg-white group-hover:bg-content2 p-0'} }}
                    description={
                        <p className='leading-tight text-[.85rem]'>{item.desc}</p>
                    }
                    name={item.name}
                />
            </div>)}
          ...
          <p style={{fontWeight:'500'}} className="text-tiny   leading-tight">{item.desc}</p>
      </div>
                </div>
            )}
        </Checkbox>
