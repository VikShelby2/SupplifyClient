...
const StoreForm = ({setAnimationTime}) => {
 ...
 const navigate = useNavigate()
  ...
  return !loading ? (
    <div className="flex items-center w-[600px] h-[400px]  bg-white rounded-xl">
      <div className="grid gap-0 w-full p-2 text-[#303030]">
      ...
      <div className="w-full flex items-center p-5">
        ...
        <form onSubmit={handleSubmit(buildStore)} className="flex items-center  flex-col w-full gap-4 ">
      <Controller
        name="storeName"
        control={control}
        render={({ field }) => (
          <Input {...field} label="Store Name" error={errors.storeName?.message} />
        )}
      />
  ...
  {filteredCountries.map((country) => (
    <AutocompleteItem key={country.name} value={country.name}>
      ...
      <Controller
        ...
        control={control}
        render={({ field }) => (
          <Input {...field} label="Store Currency" error={errors.storeCurrency?.message} readOnly />
        )}
      />

<button type="submit" className="relative group/btn flex space-x-2 items-center justify-start px-4 w-full text-black rounded-md h-10 font-medium shadow-input bg-gray-50 dark:bg-zinc-900 dark:shadow-[0px_0px_1px_1px_var(--neutral-800)]">
