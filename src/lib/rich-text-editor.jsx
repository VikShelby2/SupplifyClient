...
export default function RichTextEditor() {
  ...
  const [items, setItems] = useState([
    { name: 'Item 1', isActive: false  , icon:  <Bold className='w-5 h-5' /> , onClick: () => editor?.chain().focus().toggleBold().run()  },
    { name: 'Item 2', isActive: false , icon :  <Italic className='w-5 h-5 '/> , onClick: () => editor?.chain().focus().toggleItalic().run() },
    { name: 'Item 3', isActive: false , icon: <Underline className='w-5 h-5 ' />  ,onClick : () => editor?.chain().focus().toggleUnderline().run() },
    { name: 'Item 1', isActive: false  , icon:  <Strikethrough className='w-5 h-5' />  , onClick: () => editor?.chain().focus().toggleStrike().run()},
    { name: 'Item 2', isActive: false , icon :  <TableIcon className='w-5 h-5 '/> , onClick: addTable },
    { name: 'Item 3', isActive: false , icon: <ImageIcon className='w-5 h-5 ' /> , onClick: addImage },
  ]);
  ...
  return (
      ...
      >
      <DropdownTrigger>
      <div   className={`inline-flex mr-1  items-center justify-center whitespace-nowrap rounded-[.5rem] px-[12px] py-[4px] text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50  hover:bg-[#ebebeb] shadow-input   `}>
  ...
  {selectedValue}
      ...
      >
        <DropdownItem hideSelectedIcon    onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()}  key="Heading 1">Heading 1</DropdownItem>
        <DropdownItem hideSelectedIcon  onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} key="Heading 2">Heading 2</DropdownItem>
        <DropdownItem hideSelectedIcon   onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()} key="Heading 3">Heading 3</DropdownItem>
